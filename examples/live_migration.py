...
:meth:`skya.policies.Placement.migrate` runs once per tick (after the
schedulers). Override it to detect fragmentation and start migrations
with :meth:`~skya.policies.Placement.migrate_vm`. A migration changes
...
commits the move (``vm.migrate.done``) ``duration`` ticks later. This
example puts a dynamic-consolidation policy on top of first-fit:
  ...
  * the policy migrates svc-1 to pm-2, sizing the copy from ``vm.ram``
    via a :class:`skya.network.Link`'s ``transmission_time``
...
A small :class:`~skya.Tracker` counts migrations and each host's
final VM count, so the result appears on the standard ``sim.report``
line (``migrate.count=1``, ``host.pm-1.vms=0 host.pm-2.vms=2``).

``vm.ram`` sizes the copy because only *memory* moves between hosts:
the VM's disk stays on shared storage (SAN/NFS) and is not part of the
transfer.
...
from skya import App, Daemon, DataCenter, PM, Request, Simulation, Topic, Tracker, VM, on
from skya.network import Link
from skya.policies import FirstFit, SpaceShared, TimeShared

# The migration link: 512 "MB" of memory copied per tick, zero setup
# latency. Only transmission_time() is used here - LINK_DELIVER models
# app traffic between VMs, not infrastructure transfers.
...
class ConsolidatingPlacement(FirstFit):
    """First-fit on arrival; each tick, move a VM that is alone on its host onto a host with other VMs."""

    def migrate(self) -> int:
        if any(self.is_migrating(vm) for vm in self):       # one migration at a time keeps the example simple
            return 0
...
# A finite batch job + a service share pm-1; a second service is placed
# on pm-2 instead, because pm-1's two cores are already full.
