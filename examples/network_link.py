...
``skya.network`` is an *optional* building block: the core simulator
never creates a :class:`~skya.network.Link`, VMs and PMs have no NIC,
...
:data:`skya.Topic.LINK_DELIVER` event at
...
A small :class:`~skya.Tracker` also counts received bytes, so the
total appears on the standard ``sim.report`` line. The run is bounded
with ``run(duration=...)`` because the services never finish on their
own (a :class:`~skya.models.Daemon` runs forever); the run *pauses*
at the time limit with the VMs still placed.

(If the client ran a *finite* job instead, you would trigger the send
on :data:`skya.Topic.APP_START` - but then its VM is reclaimed when
...
from skya import DataCenter, Daemon, PM, Request, Simulation, Topic, Tracker, VM, on
