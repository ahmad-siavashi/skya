...
A :class:`skya.Tracker` extends the default per-request bookkeeping
with custom metrics. Two extension points:

1. **The :func:`skya.on` decorator** - mark any method with
...
This example counts the cumulative CPU cores allocated per host. Under
...
from skya import App, DataCenter, PM, Request, Simulation, Topic, Tracker, VM, on
