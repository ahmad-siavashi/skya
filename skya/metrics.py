"""Ready-made trackers - CPU utilization, completion time, concurrency, energy.

Each is a :class:`skya.Tracker` subclass that subscribes to the event
...
from skya.core import on
from skya.models import PM, VM
...
class Utilization(Tracker):
    ...
    for the latter see :attr:`skya.policies.Hypervisor.cpu_utilization`
...
class Energy(Tracker):
    ...
    ``host.power.power(host, host.hypervisor.cpu_utilization)``. So energy
    tracks *actual CPU work* (via :attr:`Hypervisor.cpu_utilization`),
    ...
    def _on_tick(self, _simulation, dt: float) -> None:
        for host in self.simulation.datacenter.hosts:
            watts: float = host.power.power(host, host.hypervisor.cpu_utilization)
