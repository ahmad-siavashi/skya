...
Most users create these via :meth:`skya.Simulation.create`, which
...
Skya models one cluster level: a :class:`DataCenter` holds a list of
...
- **RAM is allocated in two layers.** The default
  :class:`~skya.policies.SpaceShared` hypervisor reserves :attr:`VM.ram`
...
one of the host's cores at the host's speed, not a normalized unit.
:attr:`App.length` is in cycles, not seconds. GPUs / accelerators are
not modeled. :mod:`skya.network` is a separate, standalone module:
...
from skya.core import Clock, EventQueue
from skya.policies import Hypervisor, Placement, Scheduler
from skya.power import PowerModel, ZeroPower
...
class SimObject:
    ...
    and injects those two automatically. Plain data classes that do not
    need them (e.g. ``Request``) do not inherit from :class:`SimObject`
...
class Daemon:
    ...
    ram : int
        ...
        the VM's free RAM is rejected (see
        :meth:`skya.policies.Scheduler.schedule`). Defaults to ``0`` -
        no RAM requirement, always admitted.
    """
...
class App(Daemon):
    ...
    length : tuple[int, ...]
        ...
        10-time-unit job" you must know the core rate it will run on.
    """
...
class VM(SimObject):
    ...
    authoritative sources; the VM itself does not track it.
    ...
    ram : int
        ...
        :class:`~skya.policies.SpaceShared` hypervisor reserves it on
        ...
        *Inside* the VM, the :class:`Scheduler` (the guest OS) gives that
        ...
        custom
        :class:`~skya.policies.Hypervisor` may interpret ``ram``
    ...
    tenant : str, optional
        ...
        or isolation. ``None`` (default) means the VM has no assigned
        tenant.
    """
    ...
    def activate(self) -> VM:
        ...
        :meth:`skya.policies.Placement.migrate_vm`). The default does
        nothing; override to model boot behavior. Returns the VM for
        chaining."""
        return self
    ...
    def deactivate(self) -> VM:
        ...
        override to model teardown behavior, calling
...
class PM(SimObject):
    ...
    cpu : tuple[int, ...]
        ...
        :meth:`skya.policies.SpaceShared.select_cores`.
    ram : int
    ...
    power : PowerModel
        Per-host power model. Defaults to :class:`skya.power.ZeroPower`
        so runs that do not care about energy pay nothing extra. The
        :class:`skya.metrics.Energy` tracker integrates this over
        the run.
    """
...
class DataCenter(SimObject):
    ...
    *flat* list of hosts - there is no notion of zones, racks, regions,
...
class Request:
    ...
    and the verdict is final: a request that does not fit is *rejected*,
    not queued or retried.

    To react to acceptance or rejection, subscribe to
    :data:`skya.topics.REQUEST_ACCEPT` / ``REQUEST_REJECT`` on the
