...
package, on the logger ``skya.<name>``. When ``log=True`` (the
default), a stdout handler prints each event as
``<name>@<time> <topic> key=value …``. The line-formatting code is in
:mod:`skya.core.log`. To silence
Skya's logging, pass ``log=False`` or call::

    logging.getLogger('skya').setLevel(logging.WARNING)
"""
...
from skya.core import Clock, EventQueue, on, log
...
class Simulation:
    ...
    def __post_init__(self) -> None:
        ...
        # ``numpy.random`` themselves; Skya does not depend on NumPy.
    ...
    def _setup(self) -> None:
        ...
        # Default per-topic log subscribers (see :mod:`skya.core.log`).
    ...
    def run(self, duration: int | float | None = None) -> Simulation:
        ...
        duration : int | float, optional
            ...
            :class:`skya.models.Daemon`, so in that case ``run()``
    ...
    def _guard_against_infinite_stall(self) -> None:
        ...
        methods on :class:`~skya.policies.Scheduler`, so the engine
        never has to know about :class:`~skya.models.App` vs
        :class:`~skya.models.Daemon` directly.
        """
    ...
    def _advance_once(self, deadline: int | float | None = None) -> None:
        ...
        if target >= next_tick:
            ...
            # arrivals landing on a tick boundary do not get retroactively
