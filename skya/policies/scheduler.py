...
The :class:`Scheduler` manages every VM: it admits the
processes that run on the VM (allocating each one's RAM from the VM's
pool - see :attr:`skya.models.Daemon.ram`) and decides which
...
are available (the mechanics are in :class:`skya.core.Bound`).
...
For example templates you can adapt, see ``examples/custom_scheduler.py``.
"""
...
class Scheduler(Bound['models.VM'], ABC):
    ...
    in is :attr:`vm` (set by the framework when :class:`skya.models.VM`
    calls :meth:`attach` - see :class:`skya.core.Bound`); ``self.clock``
    ...
    def free_ram(self) -> int:
        ...
        :attr:`skya.models.VM.ram` units. Read it from a custom
    ...
    def schedule(self, processes: list[models.Daemon]) -> list[bool]:
        ...
        :attr:`~skya.models.Daemon.ram` fits in the VM's remaining
        ...
        list[bool]
            ...
            (and is now running), ``False`` if it was rejected (and
            not scheduled).
        """
        results: list[bool] = []
    ...
    def reset(self) -> Scheduler:
        ...
        Called by :meth:`skya.models.VM.deactivate` when the VM is
    ...
    def resume(self, cpu: tuple[int, ...], duration: int | float) -> list[int]:
        ...
        Of the three policies' ``resume`` methods, this one runs
        innermost: it runs *inside* a VM, so the hypervisor passes the
        per-core cycle rates that VM was granted, and it returns the
        cycles actually consumed on each of those cores.
        ...
        for process, share in zip(processes, per_process_shares):
            ...
            # This makes FCFS-style schedulers work without extra code:
            # processes whose share is all zeros (queued, not yet running)
            # do not tick and do not fire ``start`` events.
    ...
    def has_finite_process(self) -> bool:
        ...
        has a bounded cycle budget (an :class:`~skya.models.App`).
        ``False`` when the scheduler is idle or runs only infinite
        :class:`~skya.models.Daemon` s.
