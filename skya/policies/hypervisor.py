...
The :class:`Hypervisor` manages every PM: it decides whether a
...
:class:`skya.policies.Scheduler` divides the cycle rate among
...
VM's lifetime, and the guest's own :class:`skya.policies.Scheduler`
then sub-allocates it to its processes. A custom :class:`Hypervisor`
may model overcommit, OOM, and similar effects. See :mod:`skya.models`
...
(:meth:`~SpaceShared.select_cores`). For an example template you can
adapt, see ``examples/custom_hypervisor.py``.
"""
...
class Hypervisor(Bound['models.PM'], ABC):
    ...
    :class:`skya.models.PM`'s ``__post_init__`` calls :meth:`attach`
    (see :class:`skya.core.Bound`), after which ``self.host`` /
    ...
    def cpu_utilization(self) -> float:
        ...
        Sampled each :data:`skya.Topic.SIM_TICK` by the
        :class:`skya.metrics.Energy` tracker and fed to the host's
        :class:`skya.power.PowerModel`. The base implementation reports
    ...
    def allocate(self, vms: list[models.VM]) -> list[bool]:
        ...
        (:meth:`skya.models.VM.activate` / :meth:`~skya.models.VM.deactivate`)
        are fired by the :class:`skya.policies.Placement` around this
    ...
    def deallocate(self, vms: list[models.VM]) -> list[bool]:
        ...
        touch the guest itself (no :meth:`skya.models.VM.deactivate`),
        so :meth:`skya.policies.Placement.migrate_vm` can reuse it to
...
class SpaceShared(Hypervisor):
    ...
    def attach(self, host: models.PM) -> None:
        ...
        # were available - their ratio is :attr:`cpu_utilization`.
    ...
    @property
    def cpu_utilization(self) -> float:
        """See :attr:`Hypervisor.cpu_utilization` - here, the fraction of the
    ...
    def resume(self, duration: int | float) -> None:
        """See :meth:`Hypervisor.resume`. Also records, for :attr:`cpu_utilization`,
