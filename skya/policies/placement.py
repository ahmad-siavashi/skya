...
For example templates you can adapt, see ``examples/custom_placement.py``.
"""
...
class Placement(Bound['models.DataCenter'], ABC):
    ...
    :class:`skya.models.DataCenter`'s ``__post_init__`` calls
    :meth:`attach` (see :class:`skya.core.Bound`) - which also
    ...
    def allocate(self, vms: list[models.VM]) -> list[bool]:
        ...
        publish :data:`skya.Topic.VM_ALLOCATE`.
    ...
    def deallocate(self, vms: list[models.VM]) -> list[bool]:
        ...
        hypervisor reservation, runs the VM's :meth:`~skya.models.VM.deactivate`
        teardown hook, drops the placement record, and publishes
        :data:`skya.Topic.VM_DEALLOCATE`. There is no per-policy
    ...
    def pop_stopped(self) -> list[models.VM]:
        ...
        Skya's convention: a VM whose work just finished frees its host
        at the same instant. CloudSim (and other broker-based simulators)
        model a small teardown latency instead - that is a separate
        modeling choice you can layer on with a custom :class:`Placement`.
        """
    ...
    def advance_migrations(self) -> int:
        ...
        just before :meth:`migrate` - not intended to be called directly.
        ...
        for vm, (target, left) in list(self._migrations.items()):
            ...
            if vm in self._vm_pm:                          # still placed (not deallocated mid-transfer)
    ...
    def migrate(self) -> int:
        ...
        any transfer that finished this tick (so utilization and the
    ...
    def migrate_vm(self, vm: models.VM, target: models.PM, duration: int = 0) -> bool:
        ...
        bool
            ...
            ``False`` if ``vm`` is not placed, is already on ``target``, is
    ...
    def _commit_migration(self, vm: models.VM, target: models.PM) -> int:
        ...
        :data:`skya.Topic.VM_MIGRATE_DONE`. Returns ``1`` on success,
