...
disable logging, or configure the ``skya`` logger yourself.
"""
...
# :class:`skya.network.Link`, but having a template means it shows up
...
def get_logger(name: str, *, to_stdout: bool, clock: Clock) -> logging.Logger:
    """Return the logger for simulation ``name``, set up for Skya.

    Name-spaced ``skya.<name>`` so a caller can target one simulation
    or all of Skya via the parent ``skya`` logger. When ``to_stdout``
    ...
    """
    logger = logging.getLogger(f'skya.{name}')
...
class _StdoutHandler(logging.StreamHandler):
    ...
    A marker subclass so :func:`get_logger` can detect Skya's own
