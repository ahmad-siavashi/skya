...
A :class:`skya.policies.Scheduler`, :class:`~skya.policies.Hypervisor`,
:class:`~skya.policies.Placement`, or :class:`skya.Tracker` is created
on its own (``TimeShared()``, ``SpaceShared()``, a custom subclass, …)
and then *bound* to the model it serves - a :class:`~skya.VM`,
:class:`~skya.PM`, :class:`~skya.DataCenter`, or
:class:`~skya.Simulation` - by a one-line ``attach(owner)`` call the
...
that two-step pattern once, so each policy base does not repeat the
...
from skya.core.clock import Clock
from skya.core.event_queue import EventQueue


class _Owner(Protocol):
    """What :class:`Bound` needs from whatever it is attached to: an object
    that carries the simulation's clock and event queue. Every
    :class:`skya.models.SimObject` (``VM`` / ``PM`` / ``DataCenter``)
    and :class:`skya.Simulation` itself satisfies this."""
...
class Bound(Generic[Owner]):
    ...
    :meth:`attach` also subscribes every :func:`skya.on`-decorated
    ...
    reads the owner's own fields (as :class:`skya.policies.SpaceShared`
